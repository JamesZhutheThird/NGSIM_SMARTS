...
EGO_FEATURE_VECTOR_LENGTH = 22
V_FEATURE_VECTOR_LENGTH = 8
...
class CalObs:
    ...
    def cal_feature_vector(env_obs: Observation, **kwargs):
        ...
        roads_info = {'gneE01': [-1.5708606520231623, -0.2, 310.92, 8.43, 3.69], 'gneE05b': [-1.5708518823503947, -0.1, 180, 2.87], 'gneE51': [-1.5708418823503947, -0.1, 130.92, 3.50]}
        ...
        if ego.lane_id is None or ego.road_id is None:
            ego_road_idx = 0
            ego_lane_idx = 0
            ego_lane_feature_1 = 0
            ego_lane_feature_2 = 0
            ...
            if ego_road_idx == 1:
                ego_lane_feature_1 = (ego.heading - roads_info[ego_road_id][0]) * 100
                ego_lane_feature_2 = ego.position[0] * roads_info[ego_road_id][1] / roads_info[ego_road_id][2] + roads_info[ego_road_id][3] + ego.lane_index * roads_info[ego_road_id][4] - ego.position[1]
            elif ego_road_idx == 2:
                ego_lane_feature_1 = 0
                ego_lane_feature_2 = 0
            elif ego_road_idx == 3:
                ego_lane_feature_1 = (ego.heading - roads_info[ego_road_id][0]) * 100
                ego_lane_feature_2 = ego.position[0] * roads_info[ego_road_id][1] / roads_info[ego_road_id][2] + roads_info[ego_road_id][
                    3] - ego.position[1]
            elif ego_road_idx == 4:
                ego_lane_feature_1 = (ego.heading - roads_info[ego_road_id][0]) * 100
                ego_lane_feature_2 = (ego.position[0] - 180) * roads_info[ego_road_id][1] / roads_info[ego_road_id][2] + roads_info[ego_road_id][3] - ego.position[1]
            else:
                ego_road_idx = 0
                ego_lane_idx = 0
                ego_lane_feature_1 = 0
        ...
        ego_feature = np.asarray(
            [*ego_pos, ego_vL, ego_vW, ego_heading, ego_speed, *ego.linear_velocity[:2], *ego.angular_velocity[:2], *ego.linear_acceleration[:2], *ego.angular_acceleration[:2], *ego.linear_jerk[:2], *ego.angular_jerk[:2], ego_road_idx,
             ego_lane_idx, ego_lane_feature_1, ego_lane_feature_2])
        ...
        for i, v in surrounding_vehicles.items():
            ...
            v_features[i, :] = np.asarray([v_L, v_W, v_heading, v_speed, *rel_pos, distance, v_lane_idx])
